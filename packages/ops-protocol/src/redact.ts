/**
 * Output sanitizers shared by every hop that renders host output to a person. Applied
 * before anything reaches Discord, an audit file, or a result card.
 */

/** Truncates to a byte budget so a runaway log can never flood a message or a log line. */
...
export function redactText(value: string, secrets: string[] = []): string {
  ...
  redacted = redacted.replace(/\b([a-z][a-z0-9+.-]*:\/\/)([^:@\s/]+):([^@\s/]+)@/gi, '$1$2:[REDACTED]@');
  return redacted;
}

/** Convenience for JSON payloads that may embed secrets in string values. */
export function redactJson(value: unknown, secrets: string[] = []): unknown {
  return JSON.parse(redactText(JSON.stringify(value ?? null), secrets));
}
