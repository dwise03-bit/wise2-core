...
let APIManager = class APIManager {
    constructor(configService) {
        ...
        this.logger = new common_1.Logger('APIManager');
    ...
    async validateAllCredentials() {
        ...
        for (const [envVar, credential] of this.credentials) {
            ...
            if (!value) {
                ...
                if (credential.required) {
                    results.errors.push(`⚠️  MISSING REQUIRED: ${credential.name} (${envVar})`);
                }
            }
            else {
        ...
        this.logger.log(`✅ API Validation Complete: ${results.valid}/${results.total} credentials configured`);
    }
    /**
    ...
    generateInventory() {
        ...
        this.logger.log(`  Active: ${inventory.active.length} | Template: ${inventory.template.length} | Missing: ${inventory.missing.length}`);
    }
    /**
    ...
    getServicesByCategory(category) {
        return Array.from(this.apiRegistry.values()).filter((s) => s.category === category);
    }
    /**
     * Get all services
     */
     ...
     */
    getCredentialStatus(envVariable) {
        ...
        return {
            ...
            services: Array.from(this.apiRegistry.values()).map((service) => ({
                ...
                status: service.credentials.every((c) => c.validated)
                    ? 'healthy'
                    : 'degraded',
                credentialsConfigured: service.credentials.filter((c) => c.validated).length,
                credentialsRequired: service.credentials.filter((c) => c.required).length,
            })),
        };
    }
    /**
    ...
    getIntegrationMap() {
        return {
            summary: {
                total_services: this.apiRegistry.size,
                categories: Array.from(new Set(Array.from(this.apiRegistry.values()).map((s) => s.category))).length,
                last_validated: new Date().toISOString(),
            },
            services: Array.from(this.apiRegistry.values()).map((service) => ({
                id: Array.from(this.apiRegistry.keys()).find((k) => this.apiRegistry.get(k) === service),
                name: service.name,
