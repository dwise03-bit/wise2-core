...
test("production container is multi-stage, non-root, and health checked", () => { const dockerfile = read("Dockerfile"); assert.match(dockerfile, /FROM node:22-alpine AS build/); assert.match(dockerfile, /USER node/); assert.match(dockerfile, /HEALTHCHECK/); assert.match(dockerfile, /npm install --omit=dev/); });
