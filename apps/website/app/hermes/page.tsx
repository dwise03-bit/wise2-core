...
export default function HermesPage(){
 ...
 return <main className="min-h-screen overflow-x-hidden bg-[#02070d] text-[#d9f4ff]">
  <div className="relative isolate overflow-hidden px-2 py-2 sm:px-3">
   ...
   <header className="grid gap-3 border border-cyan-500/40 bg-[#04111c]/90 p-4 backdrop-blur xl:grid-cols-[1.4fr_1fr_1fr]">
   <div className="min-w-0"><h1 className="text-2xl font-black tracking-[.08em] text-white sm:text-3xl">W² WISE² HERMES</h1><p className="max-w-full break-words text-[10px] tracking-[.16em] text-cyan-300 sm:text-xs">YOUR AI OPERATOR · SECOND BRAIN · EXECUTION ENGINE</p></div>
   ...
   <div className="grid grid-cols-1 gap-2 text-[10px] sm:grid-cols-3 sm:text-xs">{['MAC','VPS','GPU'].map(x=><button type="button" onClick={()=>setInput(`Check ${x} status`)} key={x} className={`rounded border border-cyan-800 bg-black/30 p-2 text-center hover:border-cyan-500 ${focusClass}`}><span className="text-green-400">●</span> {x} ONLINE</button>)}</div>
  </header>
  <section className="mt-3 grid gap-3 xl:grid-cols-[180px_minmax(0,1.3fr)_minmax(320px,.9fr)_260px]">
   ...
   <section className="rounded border border-cyan-700 bg-[#04101a] p-4"><div className="flex justify-between border-b border-cyan-900 pb-3"><b className="text-2xl">HERMES</b><span aria-live="polite" className="text-xs text-green-400">● {status}</span></div>
