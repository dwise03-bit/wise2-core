...
export function LiveContextPanel({ items, onSync }: { items: readonly ContextItem[]; onSync: () => void }) {
  return <aside className="rounded border border-cyan-800 bg-[#04101a] p-3"><div className="mb-3 flex justify-between"><b>LIVE CONTEXT</b><button type="button" onClick={onSync} className={`text-xs text-cyan-400 hover:text-cyan-200 ${focus}`}>↻ Sync</button></div>
