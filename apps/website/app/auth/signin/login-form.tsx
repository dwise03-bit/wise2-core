...
export function SignInForm() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Same-origin Nest route via nginx `/api/v1/` — do not use api.wise2.net here
      // (CORS and NEXT_PUBLIC_API_URL have broken production email login before).
      ...
      if (!contentType.includes('application/json')) {
        setSubmitError('Sign-in is unavailable. Try Google or Discord.');
        analytics.track('form_error', { form: 'login', reason: 'non_json_login_response' });
      ...
      const user = nested.user ?? payload.user;
      const accessToken =
        nested.tokens?.accessToken ?? nested.accessToken ?? payload.accessToken;
      const refreshToken =
        nested.tokens?.refreshToken ?? nested.refreshToken ?? payload.refreshToken;
      ...
      if (user && accessToken) {
        ...
        setSubmitError('Sign-in succeeded but no session was returned.');
        setIsSubmitting(false);
        return;
      }
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <a href="/contact?topic=password-reset" className="text-xs text-wise-primary hover:text-wise-primary-hover">
                Forgot?
              </a>
            </div>
            <div className="relative">
