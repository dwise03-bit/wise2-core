  ...
  return (
    <nav className="fixed left-0 right-0 top-0 z-50 h-16 border-b border-white/10 bg-[#050607]/94 text-white backdrop-blur-md">
      <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex min-h-11 items-center gap-3 focus:outline-none focus:ring-2 focus:ring-[#8EDBFF]" aria-label="WISE² home">
          <span className="flex h-9 w-9 items-center justify-center border border-[#8EDBFF]/40 bg-[#DCE7EF] text-sm font-black text-[#050607]">W</span>
          <span className="flex flex-col leading-none"><span className="text-base font-black tracking-[0.12em]">WISE²</span><span className="hidden text-[10px] font-semibold uppercase tracking-[0.26em] text-[#8FA0AE] sm:block">Field-built systems</span></span>
        </Link>

        <div className="hidden items-center gap-1 lg:flex">
          {PRIMARY_LINKS.map((link) => <Link key={link.href} href={link.href} className="min-h-11 px-3 py-3 text-sm font-semibold text-[#B7C0CB] transition duration-200 hover:bg-white/5 hover:text-white focus:outline-none focus:ring-2 focus:ring-[#8EDBFF]">{link.label}</Link>)}
        </div>
