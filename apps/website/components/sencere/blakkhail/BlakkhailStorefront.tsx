...
export function BlakkhailStorefront() {
  ...
  return <>
    ...
    <section id="inspiration" className="border-b py-14 sm:py-20" style={{ borderColor: BLAKKHAIL.darkGold, backgroundColor: '#050505' }}><div className={BLAKKHAIL_LAYOUT.container}><div className="mb-8 flex items-end justify-between border-b pb-4" style={{ borderColor: BLAKKHAIL.darkGold }}><div><p className="text-[10px] uppercase tracking-[0.3em]" style={{ color: BLAKKHAIL.steel }}>05 / Inspiration</p><h2 className="mt-3 text-4xl uppercase tracking-[-.03em] sm:text-6xl" style={{ color: BLAKKHAIL.gold, fontFamily: 'var(--font-display)' }}>Mood & Movement</h2></div><a href="https://pin.it/2P1PUzo68" target="_blank" rel="noopener noreferrer" className="hidden text-xs uppercase tracking-[0.18em] sm:block hover:opacity-70 transition-opacity" style={{ color: BLAKKHAIL.steel }}>View collection →</a></div><div className="grid gap-6 md:grid-cols-[1fr_1.2fr]"><div className="flex flex-col justify-center"><p className="text-xs uppercase tracking-[0.3em] mb-4" style={{ color: BLAKKHAIL.steel }}>Curated from our community</p><p className="text-lg leading-7 max-w-md" style={{ color: BLAKKHAIL.neutral600 }}>Style inspiration from people who wear Blakk Hail. No filters. No perfection. Just movement and authenticity.</p><a href="https://pin.it/2P1PUzo68" target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex items-center gap-2 border-b pb-2 text-xs uppercase tracking-[0.18em]" style={{ borderColor: BLAKKHAIL.gold, color: BLAKKHAIL.gold }}>Explore on Pinterest <ArrowUpRight size={14} /></a></div><div className="relative aspect-[3/4] overflow-hidden bg-[#2A2A2A] border" style={{ borderColor: BLAKKHAIL.darkGold }}><iframe src="https://pin.it/2P1PUzo68" width="100%" height="100%" style={{ border: 'none', borderRadius: '0' }} title="Blakk Hail Style Inspiration" /></div></div></div></section>
    ...
    <section id="shop" className={`${BLAKKHAIL_LAYOUT.container} py-14 sm:py-20`}><div className="mb-8 flex items-end justify-between border-b pb-4" style={{ borderColor: BLAKKHAIL.darkGold }}><div><p className="text-[10px] uppercase tracking-[0.3em]" style={{ color: BLAKKHAIL.steel }}>02 / Shop</p><h2 className="mt-3 text-3xl uppercase tracking-[-.03em] sm:text-5xl" style={{ color: BLAKKHAIL.gold, fontFamily: 'var(--font-display)' }}>The essentials</h2></div><span className="hidden text-xs uppercase tracking-[0.18em] sm:block">{products.length} pieces</span></div><div className="grid grid-cols-2 gap-x-3 gap-y-10 sm:gap-x-6 lg:grid-cols-3">{products.map((product, index) => <Link key={product.id} href={productPath(product.slug, host)} className="group"><div className="relative aspect-[4/5] overflow-hidden bg-[#2A2A2A] border" style={{ borderColor: BLAKKHAIL.darkGold, animation: `revealProduct 0.7s ease-out ${index * 0.1}s both` }}><Image src={product.image || shop[index % shop.length]} alt={product.name} fill sizes="(max-width: 640px) 50vw, 33vw" className="object-cover transition-transform duration-700 group-hover:scale-[1.035]" /><div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all duration-500" /></div><div className="flex items-start justify-between gap-3 pt-3 text-xs uppercase tracking-[0.08em] opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ color: BLAKKHAIL.gold }}><span>{product.name}</span><span>${product.basePrice.toFixed(2)}</span></div><div className="flex items-start justify-between gap-3 pt-3 text-xs uppercase tracking-[0.08em] group-hover:opacity-0 transition-opacity duration-500" style={{ color: BLAKKHAIL.steel }}><span>{product.name}</span><span>${product.basePrice.toFixed(2)}</span></div></Link>)}</div><style jsx>{`
