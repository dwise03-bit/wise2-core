      ...
      ScrollView(showsIndicators: false) {
        VStack(alignment: .leading, spacing: 14) {
          if viewModel.messages.isEmpty {
            CommandCard {
              Text("Business operations command surface")
                .font(.headline)
                .foregroundColor(.wise2TextPrimary)
              Text("Hermes mode: \(BusinessScope.hermesMode(for: selectedBusiness)). Ask for a briefing, queue CRM/finance drafts, or approve live Hermes actions.")
                .font(.subheadline)
          ...
          if !viewModel.pendingApprovals.isEmpty {
            SectionLabel(title: "Pending Hermes Approvals")
            ForEach(viewModel.pendingApprovals) { item in
              Button {
                viewModel.openPendingApproval(item)
              } label: {
                CommandCard {
                  Text(item.title)
                    .font(.headline)
                    .foregroundColor(.wise2TextPrimary)
                  Text("\(item.status) · \(item.risk) · \(item.kind)")
                    .font(.caption)
                    .foregroundColor(.wise2TextSecondary)
                }
              }
          ...
          if let proposedAction = viewModel.proposedAction {
            ApprovalCard(action: proposedAction) {
              ...
              Task { await viewModel.rejectProposedAction() }
            } confirmFaceID: {
