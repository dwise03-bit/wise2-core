...
struct SystemsScreen: View {
  ...
  var body: some View {
    CommandSurface(title: "Systems", subtitle: "Infrastructure visibility and safe controls", selectedBusiness: selectedBusiness) {
      HStack(spacing: 12) {
        MetricBox(label: "Uptime", value: viewModel.metrics?.uptime ?? "99.98%", color: .wise2Success)
        MetricBox(label: "Latency", value: "\(viewModel.metrics?.networkLatency ?? 23)ms", color: .wise2Primary)
        MetricBox(label: "Incidents", value: "1", color: .wise2Warning)
      }
