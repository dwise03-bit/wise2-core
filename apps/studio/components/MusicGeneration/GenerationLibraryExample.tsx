...
/**
 * Integration Example: How to use in your main studio page
 *
 * In your studio app (e.g., app/studio/page.tsx):
 *
 * ```tsx
 ...
 *       {/* Navigation */}
 *       <nav className="w-20 bg-slate-900 border-r border-slate-700">
 *         <button
 ...
 *         {activeTab === 'generate' && <PromptBuilder />}
 *         {activeTab === 'library' && (
 *           <GenerationLibrary
 *             generations={generations}
 ...
 * ```tsx
 * // actions.ts
 * export const playGeneration = (track: GeneratedTrack) => ({
 *   type: 'PLAY_GENERATION',
 *   payload: track,
 * });
 *
 * export const deleteGeneration = (trackId: string) => ({
 ...
 *   onDeleteGeneration={(id) => dispatch(deleteGeneration(id))}
 *   // ... other props
 * />
 * ```
 */
 ...
 * export async function fetchGenerations(page = 1, limit = 50) {
 *   const response = await fetch(`/api/generations?page=${page}&limit=${limit}`);
 *   return response.json();
 * }
 *
 * export async function deleteGeneration(trackId: string) {
 *   await fetch(`/api/generations/${trackId}`, { method: 'DELETE' });
 * }
 *
 * export async function updateGeneration(trackId: string, updates: Partial<GeneratedTrack>) {
 ...
 *     `/api/generations/${trackId}/export?format=${format}`
 *   );
 ...
 * // component.tsx
 * useEffect(() => {
 *   fetchGenerations().then(data => setGenerations(data.generations));
 * }, []);
 *
