...
describe('control bridge server', () => {
  ...
  it('accepts valid auth and rejects bad auth without echoing tokens', async () => {
    const cfg = config();
    const app = await buildServer(cfg, { run: async () => ok() });
    const good = await app.inject({ url: '/v1/control/git/revision', headers: { authorization: `Bearer ${cfg.token}` } });
    expect(good.statusCode).toBe(200);
    const bad = await app.inject({ url: '/v1/control/git/revision', headers: { authorization: 'Bearer wrong-token' } });
    expect(bad.statusCode).toBe(401);
    expect(bad.body).not.toContain('wrong-token');
  });
  ...
  it('restarts allowlisted services with fixed docker compose arguments and audits', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'control-bridge-'));
  ...
  it('caps logs at 500 lines', async () => {
    let args: string[] = [];
    const app = await buildServer(config(), { run: async (_binary, commandArgs) => { args = commandArgs; return ok('logs'); } });
    await app.inject({ url: '/v1/control/docker/api/logs?lines=900', headers: { authorization: 'Bearer test-token-with-length' } });
  ...
  it('parses Docker compose services through the configured allowlist', async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect(res.json().data.services).toEqual(['api', 'website']);
  });
  ...
  it('returns degraded aggregate status for optional component failures', async () => {
    ...
    const app = await buildServer(config(), { run, fetch: fetch as typeof globalThis.fetch });
    const res = await app.inject({ url: '/v1/control/status', headers: { authorization: 'Bearer test-token-with-length' } });
    expect(res.statusCode).toBe(200);
    expect(res.json().data.ollama.status).toBe('down');
    expect(res.json().data.hermes.status).toBe('down');
  });

  it('reports Ollama unavailable without failing the endpoint', async () => {
    const fetch = async () => { throw new Error('ollama offline'); };
    const app = await buildServer(config(), { run: async () => ok(), fetch: fetch as typeof globalThis.fetch });
    const res = await app.inject({ url: '/v1/control/ollama/status', headers: { authorization: 'Bearer test-token-with-length' } });
    expect(res.statusCode).toBe(200);
    expect(res.json().data.status).toBe('down');
  });

  it('reports Hermes unavailable without failing the endpoint', async () => {
    const fetch = async () => { throw new Error('hermes offline'); };
    const app = await buildServer(config(), { run: async () => ok(), fetch: fetch as typeof globalThis.fetch });
    const res = await app.inject({ url: '/v1/control/hermes/status', headers: { authorization: 'Bearer test-token-with-length' } });
    expect(res.statusCode).toBe(200);
    expect(res.json().data.status).toBe('down');
  });
  ...
  it('reports wise2.net degraded state without throwing', async () => {
    ...
    const app = await buildServer(config(), { run: async () => ok(), fetch: fetch as typeof globalThis.fetch });
    const res = await app.inject({ url: '/v1/control/web/wise2', headers: { authorization: 'Bearer test-token-with-length' } });
    expect(res.statusCode).toBe(200);
    expect(res.json().data.public.status).toBe('degraded');
  });
  ...
  it('rate limits requests', async () => {
    const app = await buildServer(config({ rateLimitMax: 1 }), { run: async () => ok() });
    const headers = { authorization: 'Bearer test-token-with-length' };
    expect((await app.inject({ url: '/v1/control/git/status', headers })).statusCode).toBe(200);
    expect((await app.inject({ url: '/v1/control/git/status', headers })).statusCode).toBe(429);
  });
  ...
  it('never includes the bearer token in command responses', async () => {
    const cfg = config();
    const app = await buildServer(cfg, { run: async () => ok(cfg.token) });
    const res = await app.inject({ url: '/v1/control/docker/api/logs', headers: { authorization: `Bearer ${cfg.token}` } });
    expect(res.body).not.toContain(cfg.token);
  });
});
