...
Launch via the driver script at `.claude/skills/run-wise2-core/driver.sh`:

```bash
# Start Docker stack (PostgreSQL, Redis) + API server in background
bash .claude/skills/run-wise2-core/driver.sh start

# Verify API is responding (retry loop until health check passes)
bash .claude/skills/run-wise2-core/driver.sh health-check

# Take a screenshot of the running status
bash .claude/skills/run-wise2-core/driver.sh status

# Stop everything
bash .claude/skills/run-wise2-core/driver.sh stop
```

**Screenshot location:** `.claude/skills/run-wise2-core/status.txt` (text output)
...
**Driver location:** `.claude/skills/run-wise2-core/driver.sh`  
**Status check output:** `.claude/skills/run-wise2-core/status.txt`
