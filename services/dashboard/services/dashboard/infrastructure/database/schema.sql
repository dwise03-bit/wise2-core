...
CREATE INDEX idx_deployment_services_status ON deployment_services(status);
CREATE INDEX idx_deployment_services_service_type ON deployment_services(service_type);
...
CREATE INDEX idx_services_is_healthy ON services(is_healthy);
...
CREATE INDEX idx_automation_jobs_is_enabled ON automation_jobs(is_enabled);
...
-- Grant permissions to app user (if using separate app user)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_user WHERE usename = 'wise2_app') THEN
    ...
    GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA public TO wise2_app;
  END IF;
END
$$;
