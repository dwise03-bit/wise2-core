...
class StudioAI:
    ...
    Generates music from prompts using custom MusicGen model (replaces Suno)
    Transfers styles, manages live streams
    """

    def __init__(self, ollama_url: str = "http://127.0.0.1:11434", musicgen_api_url: str = "http://173.208.147.165:4900", vps_api_url: str = "https://wise2.net/api/v1"):
        self.ollama_url = ollama_url
        self.musicgen_api_url = musicgen_api_url  # Custom MusicGen-small API
    ...
    async def generate_music(
        ...
        """
        Generate music from text prompt using custom MusicGen-small model
        Replaces Suno with proprietary WISE² implementation
        """
        try:
            job_id = f"music_{datetime.now().timestamp()}"

            # Enrich prompt with style + genre info
            enriched_prompt = f"{prompt}, {style.value} music, {bpm} bpm"
            ...
            # Generate music using custom MusicGen API
            generation_task = asyncio.create_task(
                self._process_generation(job_id, enriched_prompt, style, duration_seconds, bpm, key)
            )
    ...
    async def _process_generation(
        ...
        """
        Process music generation using custom MusicGen-small API
        Calls WISE² custom model endpoint (replaces Suno)
        """
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    # Parse response (audio_url, request_id, etc.)
                    music_data = {
                        ...
                        "instruments": ["MusicGen-small (fine-tuned)"],
                        "production": f"Generated by WISE² custom MusicGen model (Phase 1)",
                    ...
                    logger.info(f"✅ Generated music via custom model: {job_id}")
                    return music_data
                else:
                    logger.error(f"Custom MusicGen API failed: {response.status_code}")
                    return None
    ...
    def _infer_mood(self, prompt: str) -> str:
        ...
        if any(word in prompt_lower for word in ["calm", "peaceful", "ambient", "relax", "meditation"]):
            return "peaceful"
        elif any(word in prompt_lower for word in ["energetic", "fast", "dance", "upbeat", "party"]):
            return "energetic"
        elif any(word in prompt_lower for word in ["sad", "melancholy", "slow", "ballad", "emotional"]):
            return "melancholic"
        elif any(word in prompt_lower for word in ["epic", "orchestral", "cinematic", "dramatic"]):
            return "dramatic"
        else:
            return "neutral"

    def _parse_music_generation(self, response: str) -> Dict[str, Any]:
        """Parse custom MusicGen API response"""
        try:
