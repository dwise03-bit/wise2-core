...
@Composable private fun LoginScreen(onLogin:(Wise2Session)->Unit){
    ...
    Column(Modifier.fillMaxSize().background(Bg).padding(28.dp),verticalArrangement=Arrangement.Center){
        ...
        Text("CJAYS",fontSize=34.sp,fontWeight=FontWeight.Black,modifier=Modifier.padding(top=18.dp));Text("AUTO RECON SOLUTIONS · POWERED BY WISE²",color=Blue,fontSize=12.sp,fontWeight=FontWeight.Bold)
        Text("Sign in to your Wise² client workspace",fontSize=18.sp,fontWeight=FontWeight.Bold,modifier=Modifier.padding(top=34.dp,bottom=14.dp))
    ...
    LazyColumn(contentPadding=PaddingValues(bottom=24.dp)) {
        ...
        item {
            Column(Modifier.padding(horizontal=20.dp),verticalArrangement=Arrangement.spacedBy(12.dp)) {
                when(stage){
                    0->{
                        BarcodeScannerView(onDetected={ detected -> vin=detected },onManual={})
                        Field("VIN",vin,{vin=it.uppercase().filter{c->c.isLetterOrDigit()}.take(17)},KeyboardType.Ascii)
                        Button(onClick={ if(vin.length!=17){error="VIN must contain 17 characters."} else { val found=data.vehicles.find{it.vin==vin}; if(found!=null){year=found.year;make=found.make;model=found.model;error="VEHICLE FOUND — existing history loaded";stage=1} else { decoding=true;error="";scope.launch { VinDecoder.decode(vin).onSuccess{decoded->year=decoded.ModelYear;make=decoded.Make;model=decoded.Model}.onFailure{error="VIN decoding unavailable. Enter vehicle details manually."};decoding=false;stage=1 } } } },enabled=!decoding,modifier=Modifier.fillMaxWidth().height(54.dp),shape=RoundedCornerShape(12.dp)){if(decoding)CircularProgressIndicator(Modifier.size(20.dp),strokeWidth=2.dp,color=Color.White)else Text("SEARCH & DECODE VIN",fontWeight=FontWeight.Black)}
                    }
                    ...
                    3->{ Field("Service",service,{service=it});Field("Price",price,{price=it},KeyboardType.Decimal);Primary("START JOB"){
                        ...
                        onChange(data.copy(customers=if(existingCustomer==null)data.customers+customer else data.customers,vehicles=if(existingVehicle==null)data.vehicles+vehicle else data.vehicles,jobs=data.jobs+job)); stage=0;vin="";name="";phone="";year="";make="";model="";price=""
                    }}
                }
...
@Composable private fun JobWorkflow(job:Job,data:AppData,token:String,onUpdate:(Job)->Unit,onBack:()->Unit){
    ...
    var aiSuggestion by remember{mutableStateOf("")};var aiBusy by remember{mutableStateOf(false)};val scope=rememberCoroutineScope()
    ...
    LazyColumn(contentPadding=PaddingValues(bottom=28.dp)){
        ...
        item{SectionTitle("Wise² AI assistant");Column(Modifier.padding(horizontal=20.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){Button(onClick={aiBusy=true;scope.launch{Wise2Client.aiAssist(token,current.id,"quality_review").onSuccess{aiSuggestion=it.suggestion}.onFailure{message=it.message?:"AI unavailable"};aiBusy=false}},enabled=!aiBusy,modifier=Modifier.fillMaxWidth()){Icon(Icons.Default.AutoAwesome,null);Spacer(Modifier.width(8.dp));Text(if(aiBusy)"ANALYZING…" else "REVIEW THIS JOB")};if(aiSuggestion.isNotBlank()){Surface(color=Raised,shape=RoundedCornerShape(12.dp)){Text(aiSuggestion,modifier=Modifier.padding(14.dp),color=Ink)};OutlinedButton(onClick={scope.launch{Wise2Client.saveToDrive(token,"CJAYS-${current.id}-AI-review.txt",aiSuggestion).onSuccess{message="Approved AI review saved to Google Drive."}.onFailure{message=it.message?:"Drive export failed"}}},Modifier.fillMaxWidth()){Icon(Icons.Default.AddToDrive,null);Spacer(Modifier.width(8.dp));Text("APPROVE & SAVE TO DRIVE")};Text("AI suggestions require human review. Nothing is sent automatically.",color=Muted,fontSize=11.sp)}}}
        item{SectionTitle("Payment");Column(Modifier.padding(horizontal=20.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){Field("Paid amount",current.paidAmount,{current=current.copy(paidAmount=it);onUpdate(current)},KeyboardType.Decimal);Field("Payment method",current.paymentMethod,{current=current.copy(paymentMethod=it)});Button(onClick={onUpdate(current);message="Payment recorded. No electronic charge was processed."},Modifier.fillMaxWidth()){Text("RECORD PAYMENT")}}}
