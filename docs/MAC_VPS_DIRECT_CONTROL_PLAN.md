...
| **F5-OPS-08** | Health polling → `#fable5-activity` | Relay polls each target's `/v1/control/status` on an interval and posts state changes only. **Alerts never trigger writes** — the card's "Next action" is advisory. | Simulated outage posts one alert, zero automatic remediation. |
