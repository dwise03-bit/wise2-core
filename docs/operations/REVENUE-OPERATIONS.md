...
For every active prospect/client, the operating layer should be able to determine lifecycle stage, opportunity value, next best action, owner, target date or trigger, selected package/offers, payment state, build state, QA state, deployment state, onboarding state, support state, recurring-revenue state, and blocker reason.
