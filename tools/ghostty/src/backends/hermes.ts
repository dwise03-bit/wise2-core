...
export async function hermesChat(cfg: WiseConfig, prompt: string, fetchImpl = fetch): Promise<string> {
  ...
  const response = await fetchImpl(`${cfg.hermesUrl.replace(/\/$/, '')}/v1/chat/completions`, {method:'POST', headers:{'content-type':'application/json'}, signal: AbortSignal.timeout(cfg.requestTimeoutMs), body:JSON.stringify({messages:[{role:'user',content:prompt}],stream:false})});
